    ...
    Tabletop.init( { key: publicSpreadsheetUrl1,
      ...
      loadArray(data);
       ...
       for (i = 0; i < data.length; i++) {
             //console.log(i)
           pos = i + 1;
content += '<tr><td class="nowrap">'+data[i].year+'</td><td class="financial">£'+data[i].transfer_allowance+'</td><td class="financial">£'+data[i].estimated_cost_of_training+'</td><td class="financial">£'+data[i].actual_cost_of_training+'</td><td class="financial">£'+data[i].transfer_allowance_remaining+'</td></tr>';
           renderTable(content);
            }
        }   
